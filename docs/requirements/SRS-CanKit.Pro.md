# Software Requirements Specification – CanKit.Pro Protokoll-Stacks
...
[CanKit.Pro.legacy](https://github.com/dborgards/CanKit.Pro.legacy) (Bezug: CanKit `master` @ `36866ff`,
...
> kein Fork mehr: L0/L1 sind [CanKit](https://github.com/pkuyo/CanKit), werden als NuGet-Pakete
...
- `docs/reviews/2026-09-26-canopen-master-tool-scope.md` – Master-/Tool-Rolle (#131): Entscheidungen vom 26.09.; Frage 3 (Live-Mapping und COB-ID) ist beantwortet, die übrigen Entscheidungsfragen sind offen, deshalb noch keine FR-CO-Zeile
...
| A-2 | *Überholt.* Der ISO-TP-Prototyp wurde nicht weiterverwendet; L3 ist als `CanKit.Pro.IsoTp` neu gebaut, `IsoTpScheduler` existiert nicht mehr. Die Annahme bleibt als Datensatz stehen, weil A-Nummern anderswo referenziert werden. |
...
Ist-Zustand (15.09.2026): ausgeliefert als `CanKit.Pro.IsoTp`. Die folgenden Anforderungen beschreiben das Zielverhalten; welche davon durch einen Test belegt sind, sagt `eng/verify-requirements-traceability.py`.
...
| FR-TP-032 | Das System MUSS die Zeitüberwachungen der Gegenstelle nach J1939-21 §5.10.2.4 (T1–T4) prüfen und Verbindungsabbrüche (Connection Abort) bei Überschreitung auslösen. Tr (200 ms) ist die eigene Antwortfrist eines Knotens, keine Überwachung der Gegenstelle, und wird nicht als Zeitüberwachung geführt; Th (500 ms) ist die Haltezeit zwischen CTS(0)-Nachrichten. | Must | Integrationstest: ausbleibende Gegenstelle löst Abort innerhalb der Spezifikationsfristen aus. | SAE J1939-21 |
...
Ist-Zustand (26.09.2026): ausgeliefert als `CanKit.Pro.CANopen`. Der Zuschnitt der Geräterolle ist am 16.09.2026 entschieden (`docs/reviews/2026-09-15-canopen-scope.md`) und als FR-CO-013..024 umgesetzt; der EDS/DCF-Pfad daraus (Posten 1, 2, 13 und die EDS-Hälfte von 26) folgte mit `EdsDcfNet` 1.13.0 als FR-CO-025..028 (#132). Die Master-/Tool-Rolle ist am 26.09. entschieden, soweit `docs/reviews/2026-09-26-canopen-master-tool-scope.md` (#131) das festhält: Lesen eines fremden Knotens nur gegen seine EDS oder DCF und nur für Objekte daraus; Beobachten fremder PDOs über `ObserveForeignPdoAsync` (#163): live `1600h`–`1603h` und `1A00h`–`1A03h` sowie live `1400h:01` / `1800h:01`, soweit `SdoUploadAsync` das Paar überträgt; eine Verweigerung des Peer-SDO-Tors (#174), ein Abbruch oder eine Zeitüberschreitung nimmt die an den Aufruf übergebene Datei; Anwesenheit primär durch Hören von Heartbeat und Boot-up (etwa 1–2 Sekunden), ein aktiver Scan der Node-IDs 1..127 nur auf Abruf und ohne die eigene Node-ID; Flying Master ist Pflicht und nicht mehr optional. Mit #164 ist die Wahl des aktiven Masters umgesetzt, unter der Annahme CiA 302-2 (historisch DSP 302 Abschnitt 5.5); Ausgabe und Abschnitt sind nicht an einem Mitgliedstext geprüft und bleiben Frage 4. Der Boot-up-Manager ist nicht Teil dieser Umsetzung (Frage 5). Diese Fähigkeiten sind noch nicht als FR-CO ausformuliert, weil die offenen Entscheidungsfragen dort den Ablauf nicht hergeben. Die Geräteanforderungen FR-CO-013..028 bleiben fachlich richtig. In der Aufmerksamkeit treten die Fallback-Records ohne EDS hinter die genannten Master-Lücken zurück; welche weiteren Geräteposten das ebenfalls tun, ist dort Frage 7. Die Posten-Nummern in der Spalte Quelle verweisen auf die Tabelle „Die Posten“ in `docs/reviews/2026-09-15-canopen-scope.md`. „Architekturentscheidung“ steht dort, wo CiA 301 das Verhalten nicht verlangt — Übertragungsarten und Inhibit Time sind Architektur, nicht Norm; wo die Norm nur die Ablehnung, die Semantik oder einen Abort-Code beisteuert, sagt die Zelle das.
...
| FR-CO-013 | Das System MUSS beim Öffnen eines Knotens die Kommunikationsprofil-Objekte 1000h, 1001h, 1018h (sub0 = 01h, Vendor-ID 0 = „keine Vendor-ID zugeteilt“), 1005h, 1006h, 100Ch, 100Dh, 1010h, 1011h, 1014h, 1016h, 1017h, 1200h sowie die PDO-Records 1400h–1403h, 1600h–1603h, 1800h–1803h und 1A00h–1A03h mit ihren CiA-301-Vorgabewerten im OD anlegen (PDOs „nicht gültig“, Bit 31 gesetzt). Ohne die Option `WritableCommunicationParameters` MÜSSEN die PDO-Records auf dem Bus `ro` sein (Download → 0601 0002h); 1200h ist konstant; der reservierte Sub-Index 04h der PDO-Kommunikationsrecords MUSS fehlen, so dass ein Zugriff 0609 0011h ergibt. | Must | Virtual-Loopback-Integrationstest (`CanOpenCommunicationProfileTests`): SDO-Upload jedes Objekts liefert den Vorgabewert; Download auf 1800h:02 ohne Option → 0601 0002h, mit Option angenommen; Zugriff auf 1800h:04 → 0609 0011h. | CiA 301 §7.5.2.1, §7.5.2.2, §7.5.2.21 und Objektübersicht `ro M` (Posten 3); Fußnote `*` der Objektübersicht (Posten 4, 16); §7.5.2.5, §7.5.2.6, §7.5.2.17 (Posten 19); §7.5.2.11, §7.5.2.12 (Posten 24); §7.5.2.37 zu Sub-Index 04h (Posten 18); Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 4b, 21) für 1200h und 1016h/1017h |
| FR-CO-014 | Das System MUSS das OD als einzige Quelle der Kommunikationskonfiguration führen, in beiden Richtungen und auf beiden Schreibpfaden: ein Wert, den das OD für ein vom Knoten verwaltetes Objekt annimmt — per SDO-Download wie per lokalem Schreibzugriff über `ObjectDictionary` —, MUSS im zugehörigen Dienst (SYNC, EMCY, Heartbeat, Life Guarding, PDO) wirken; `StartHeartbeatProducer`, `AddHeartbeatConsumer`, `StartSyncProducer`, `SendEmcyAsync` (1001h), `ConfigureTpdo` und `ConfigureRpdo` MÜSSEN als Schreibzugriffe auf diese Objekte ausgeführt werden; ein nicht umsetzbarer Wert MUSS vor dem Speichern abgewiesen werden (SDO: Abort-Code, lokal: `ArgumentException` mit demselben Code im Text); ein verwaltetes Objekt darf per `Add*` nicht neu deklariert werden (`InvalidOperationException`). | Must | Virtual-Loopback-Integrationstest (`CanOpenCommunicationProfileTests`): 1017h per SDO gesetzt startet den Heartbeat; `StartSyncProducer` schreibt 1006h und Bit 30 von 1005h; `SendEmcyAsync` spiegelt 1001h; `AddU32(0x1006, …)` wirft; lokaler Schreibzugriff mit ungültigem Wert wirft mit dem Abort-Code im Text. | Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 14, 20, 22, 23) |
| FR-CO-015 | Das System MUSS für TPDOs die Übertragungsarten aus 1800h:02 umsetzen: 00h synchron-azyklisch (Senden nach dem nächsten SYNC, sofern seit dem vorigen SYNC eine Zustandsänderung eines gemappten Objekts oder `TriggerTpdoAsync` auftrat), 01h–F0h synchron-zyklisch (bei jedem n-ten SYNC), FCh RTR-only synchron (Abtastung bei jedem SYNC, der gepufferte Wert antwortet auf ein RTR), FDh RTR-only ereignisgesteuert (Abtastung und Senden bei RTR), FEh/FFh ereignisgesteuert (Zustandsänderung, `TriggerTpdoAsync`, Event-Timer). `TpdoTransmission` MUSS auf diese Bytes abbilden (`EventDriven`/`EventTimer` → FEh, `Synchronous` → 01h); 02h–F0h wird als Rohbyte in 1800h:02 geschrieben. Ein Download eines reservierten Werts (TPDO F1h–FBh, RPDO F1h–FDh) MUSS mit 0609 0030h abgewiesen werden. | Must | Virtual-Loopback-Integrationstest (`CanOpenPdoEngineTests`): je Übertragungsart die erwartete Frame-Folge über SYNC, RTR und Trigger (03h sendet beim dritten SYNC; FCh antwortet erst nach dem ersten SYNC); Download F1h → 0609 0030h. | Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 5, 6a, 6b, 11); die Ablehnung reservierter Werte ist normativ: CiA 301 §7.5.2.37 Table 72, §7.5.2.35 Table 68 (Posten 17) |
| FR-CO-016 | Das System MUSS die Inhibit Time 1800h:03 (Vielfache von 100 µs) als Mindestabstand zweier Sendungen eines ereignisgesteuerten TPDO (FEh/FFh) einhalten — ein Ereignis innerhalb der Sperrzeit führt zu genau einer Sendung nach deren Ablauf, weitere Ereignisse bis dahin fallen mit ihr zusammen — und den Event-Timer 1800h:05 (Vielfache von 1 ms) als Höchstabstand: er startet mit jeder Sendung neu und sendet nur im Zustand Operational. Beide gelten nicht für synchrone und RTR-only-Arten; 0 schaltet ab; 1800h:03 darf nur geändert werden, solange das PDO ungültig ist (Bit 31 = 1), sonst 0609 0030h. | Must | Unit-Test mit virtueller Uhr und Virtual-Loopback-Integrationstest (`CanOpenPdoEngineTests`): zwei Zustandsänderungen innerhalb der Inhibit Time ergeben zwei Frames im Abstand ≥ Inhibit Time; ohne Ereignis folgt nach 1800h:05 ein Frame; Download auf 1800h:03 bei gültigem PDO → 0609 0030h. | Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 7); Semantik und Einheiten: CiA 301 §7.5.2.37 (Sub-Index 03h, 05h) |
| FR-CO-017 | Das System MUSS ein RPDO mit 1400h:02 = 00h–F0h synchron behandeln: empfangene Daten werden gehalten und erst mit dem nächsten SYNC ins OD geschrieben und über `RpdoReceived` gemeldet (bei mehreren Empfängen vor dem SYNC der jüngste); FEh/FFh wirkt sofort. `ConfigureRpdo` MUSS die Art als `RpdoTransmission` entgegennehmen. | Must | Virtual-Loopback-Integrationstest (`CanOpenPdoEngineTests`): RPDO-Frame vor SYNC lässt das OD unverändert, der SYNC schreibt es und hebt `RpdoReceived`. | Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 12); Semantik: CiA 301 §7.2.2.2, §7.5.2.35 Table 68 |
| FR-CO-018 | Das System MUSS die COB-ID-Einträge 1400h:01, 1800h:01, 1005h und 1014h als 32-Bit-Steuerwörter auswerten: Bit 31 *valid* (1400h:01, 1800h:01: gesetzt = das PDO wird aus der Laufzeit entfernt; 1014h: gesetzt = EMCY-Senden wird verweigert), Bit 30 *RTR* in 1800h:01 (gesetzt = kein RTR wird beantwortet), Bit 30 *gen.* in 1005h (SYNC-Produzent ein/aus), Bit 30 von 1014h reserviert (gesetzt → 0609 0030h), Bit 29 *frame* (29-Bit-ID) → 0609 0030h. Die Bits 0–29 dürfen nicht geändert werden, solange das Objekt gültig ist (1005h: solange Bit 30 = 1), sonst 0609 0030h; eine CAN-ID aus Table 40 (restricted) MUSS mit 0609 0030h abgewiesen werden, lokal mit `ArgumentException`. | Must | Virtual-Loopback-Integrationstest (`CanOpenCommunicationProfileTests`): Bit 31 auf 1800h:01 gesetzt stoppt das TPDO, gelöscht startet es; Download mit Bit 29, mit CAN-ID 000h/07Fh/581h/701h und mit ID-Wechsel bei gültigem PDO → 0609 0030h. | CiA 301 §7.5.2.5 Table 55, §7.5.2.17 Table 59, §7.5.2.35 Table 66, §7.5.2.37 Table 70, §7.3.5 Table 40 (Posten 25, 27; #41) |
| FR-CO-019 | Das System MUSS bei NMT Reset Node und Reset Communication die verwalteten Kommunikationsprofil-Objekte (ohne 1001h, 1010h, 1011h, 1200h) auf ihre Power-on-Werte zurücksetzen — die zuletzt über 1010h:01 „save“ bzw. `StoreParameters()` gesicherten Werte, sonst die Vorgabewerte beim Öffnen —, laufende SDO-Server-Sitzungen mit 0800 0022h abbrechen, den Boot-up senden und in Pre-Operational übergehen; beide Kommandos setzen dieselbe Menge zurück, weil ohne Gerätebeschreibung keine Quelle für Anwendungsobjekte existiert (diese setzt die Anwendung über `ApplicationReset` selbst zurück, das synchron vor dem Boot-up läuft). 1011h:01 „load“ bzw. `RestoreDefaultParameters()` MUSS bewirken, dass erst der nächste Reset die Vorgabewerte wiederherstellt; eine falsche Signatur auf 1010h:01 oder 1011h:01 MUSS mit 0800 0020h abgewiesen werden. Die Speicherung ist flüchtig (Prozesslebensdauer). | Must | Virtual-Loopback-Integrationstest (`CanOpenCommunicationProfileTests`): `ConfigureTpdo` → Reset Communication → 1800h:01 wieder ungültig; nach `StoreParameters()` bleibt die Konfiguration; nach `RestoreDefaultParameters()` erst mit dem Reset zurück auf die Vorgabe; falsche Signatur → 0800 0020h. | CiA 301 §7.3.2.2.1, §7.5.2.13, §7.5.2.14 (Posten 15) |
| FR-CO-020 | Das System MUSS jeden Mapping-Eintrag — per SDO in 1600h/1A00h wie per `ConfigureTpdo`/`ConfigureRpdo` — vor dem Speichern prüfen: das Ziel existiert (sonst 0602 0000h); es ist PDO-mappbar (`OdEntry.PdoMappable`), in PDO-Richtung zugreifbar (TPDO lesbar, RPDO schreibbar) und hat die gemappte Breite (sonst 0604 0041h); die Bitlänge ist ein Vielfaches von 8 bis 64 (sonst 0607 0010h); die Summe überschreitet 8 Byte nicht (sonst 0604 0042h); Einträge sind nur bei sub0 = 0 änderbar (sonst 0601 0000h); sub0 ist UNSIGNED8 mit Wertebereich 0..8 (41h–FFh → 0609 0030h, 9..40h → 0604 0042h); die Lage der Nutzdaten folgt der Sub-Index-Reihenfolge; ein Dummy-Eintrag 0002h–0007h (Sub-Index 0) belegt die Breite seines Datentyps ohne OD-Zugriff. Bei `ConfigureTpdo`/`ConfigureRpdo` MUSS eine Ablehnung als `ArgumentException` gemeldet werden und das PDO ungültig bleiben. | Must | Unit-Test und Virtual-Loopback-Integrationstest (`CanOpenPdoEngineTests`): je Regel ein abgelehnter Download mit dem genannten Code; außer der Reihe geschriebene Einträge liegen an ihrer Sub-Index-Position; ein Dummy-Eintrag verschiebt die Folgebytes; `ConfigureTpdo` auf ein fehlendes Objekt wirft. | CiA 301 §7.5.2.36, §7.5.2.38 (Posten 26, 28; #40) |
| FR-CO-021 | Das System MUSS produzentenseitiges Life Guarding aus 100Ch (Guard Time, ms) und 100Dh (Life Time Factor) umsetzen: das Guarding beginnt mit dem ersten empfangenen Guarding-RTR; bleibt der nächste RTR länger als Guard Time × Life Time Factor aus, MUSS `LifeGuardingEvent` mit `Occurred` gemeldet werden (einmal je Ausfall), ein späterer RTR meldet `Resolved`; 0 in einem der beiden Objekte schaltet ab. Bei aktivem Heartbeat-Producer (1017h ≠ 0) MUSS der Knoten Guarding-RTRs unbeantwortet lassen und kein Life Guarding führen. | Must | Virtual-Loopback-Integrationstest (`CanOpenLifeGuardingAndNmtStateTests`): Consumer pollt und stoppt → `Occurred`; pollt erneut → `Resolved`; mit 1017h ≠ 0 keine Guarding-Antwort. | CiA 301 §7.2.8.2.2, §7.2.8.2.2.2, §7.2.8.3.2.1, §7.2.8.3.2.2, §7.5.2.11, §7.5.2.12 (Posten 24; #43) |
| FR-CO-022 | Das System MUSS die Zuordnung von NMT-Zustand und Kommunikationsobjekten (Table 37) einhalten: PDOs werden nur in Operational gesendet und verarbeitet; in Stopped werden SDO-Anfragen nicht bedient und laufende Server-Sitzungen mit 0800 0022h abgebrochen, SYNC weder gesendet noch verarbeitet und ein `SendEmcyAsync` zurückgehalten (der jüngste EMCY geht beim Verlassen von Stopped hinaus); Heartbeat und Guarding bleiben in jedem Zustand aktiv. Der Heartbeat bei einem Zustandswechsel darf nur bei aktivem Producer (1017h ≠ 0) gesendet werden; der Guarding-Toggle MUSS bei Reset Communication (und Reset Node) auf 0 zurückgesetzt werden. | Must | Virtual-Loopback-Integrationstest (`CanOpenLifeGuardingAndNmtStateTests`): SDO-Anfrage in Stopped bleibt unbeantwortet, eine laufende Sitzung erhält 0800 0022h; EMCY in Stopped erst nach Start auf dem Bus; Stop ohne Producer sendet keinen Heartbeat; Toggle nach Reset wieder 0. | CiA 301 §7.3.2.2.4, §7.3.2.2.5 Table 37, §7.2.8.3.2.1, §7.2.8.3.2.2 (#59, #43) |
| FR-CO-023 | Das System MUSS Heartbeat-Consumer und -Producer aus 1016h und 1017h ableiten: jeder Sub-Index von 1016h mit Node-ID 1..127 und Zeit ≠ 0 (ms) ist ein Consumer, dessen Timeout sofort scharf ist; `AddHeartbeatConsumer` belegt den Sub-Index, der diesen Producer schon überwacht, sonst einen freien, sonst wächst 1016h um einen Sub-Index (sub0 steigt, höchstens 7Fh); ein zweiter Sub-Index mit Zeit ≠ 0 für dieselbe Node-ID MUSS mit 0604 0043h abgewiesen werden; 1017h (ms) startet bzw. stoppt (0) den Producer. | Must | Virtual-Loopback-Integrationstest (`CanOpenCommunicationProfileTests`): 1016h:01 per SDO gesetzt → `HeartbeatTimeout` ohne Producer; zweiter Sub-Index mit derselben Node-ID → 0604 0043h; 1017h = 0 stoppt den Producer; `AddHeartbeatConsumer` für einen zweiten Producer erhöht sub0. | Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 21); Semantik und Abort-Code: CiA 301 §7.5.2.19, §7.5.2.20 |
| FR-CO-024 | Das System MUSS ein RPDO, dessen Frame weniger Datenbytes trägt als gemappt, unverarbeitet lassen und — bei gültigem 1014h — ein EMCY mit Fehlercode 8210h senden; trägt der Frame mehr Datenbytes als gemappt, MÜSSEN die ersten Bytes bis zur gemappten Länge verwendet werden. | Must | Virtual-Loopback-Integrationstest (`CanOpenPdoEngineTests`): 1-Byte-Frame auf ein 2-Byte-RPDO lässt das OD unverändert und erzeugt EMCY 8210h; 4-Byte-Frame schreibt die ersten 2 Bytes. | CiA 301 §7.5.2.36 |
| FR-CO-025 | Das System MUSS einen Knoten aus einer Gerätebeschreibung nach CiA 306 (EDS oder DCF, gelesen mit `EdsDcfNet`) öffnen können: die Applikationsobjekte der Datei werden mit Datentyp, Zugriffsart, `PDOMapping`-Attribut und Wert im OD angelegt; die verwalteten Kommunikationsobjekte (1000h, 1001h, 1005h, 1006h, 100Ch, 100Dh, 1014h, 1016h, 1017h, 1018h) übernehmen Zugriffsart und Wert der Datei über denselben validierten Schreibpfad wie ein SDO-Download; die PDO-Records werden in der Reihenfolge von §7.5.2.38 angelegt (Kommunikationsparameter bei zerstörtem PDO, dann Mapping, dann „PDO anlegen“), und PDOs, die die Datei nicht deklariert, existieren nicht (Upload → 0602 0000h). `$NODEID+…`-Ausdrücke werden gegen die Node-ID des Knotens ausgewertet; bei einer DCF gilt `ParameterValue` vor `DefaultValue`, und ihre `NodeID` ist die des Knotens, wenn der Aufruf keine nennt. Die Laufzeit folgt dem geladenen OD wie jedem anderen (FR-CO-014). | Must | Virtual-Loopback-Integrationstest (`CanOpenDeviceDescriptionTests`): `device.eds` liefert Typ, Zugriff, Mappbarkeit und Wert von 1000h, 1018h, 2000h, 2002h sowie 1005h, 1006h, 1014h, 1016h, 1017h, 1400h, 1600h, 1800h und 1A00h wie beschrieben, 1401h/1801h/1A01h fehlen (0602 0000h); Heartbeat- und SYNC-Producer laufen, TPDO1 sendet 2000h, RPDO1 schreibt 2001h; `device.dcf` liefert Node-ID 5 und den `ParameterValue`. | CiA 306 (EDS/DCF); CiA 301 §7.5.2.38 (Reihenfolge), §7.3.3 („PDO existiert nicht“); Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 1); #132 |
| FR-CO-026 | Das System MUSS jeden Eintrag einer Gerätebeschreibung, den es nicht wie beschrieben umsetzen kann, degradieren, die Degradation im OD korrigieren und in `ICanOpenNode.DeviceDescription` (`DeviceDescriptionReport`) je Eintrag mit Index, Sub-Index, Ergebnis, Begründung, beschriebenem Wert und — wo eine SDO-Regel entschieden hat — dem Abort-Code melden; nichts wird still übergangen. Ergebnisse: *Omitted* (nicht anlegbar: Datentyp ohne Entsprechung, fünftes PDO, unparsbarer Wert, nicht implementierter Sub-Index eines festen Records), *Corrected* (der Wert wurde von derselben Regel wie ein SDO-Download abgewiesen, die CiA-301-Vorgabe bleibt), *PdoDisabled* (das PDO bleibt zerstört, weil Kommunikationsrecord oder Mapping nicht anwendbar ist), *NotImplemented* (der Eintrag existiert als Datum ohne Verhalten, etwa 1012h oder ein abweichendes 1200h), *SuppliedDefault* (ein fehlendes Pflichtobjekt 1000h/1001h/1018h behält den Platzhalter). `IsExact` ist genau dann wahr, wenn es keine Meldung gibt. | Must | Unit-Test (`CanOpenDeviceDescriptionTests`): `quirky.eds` erzeugt je Fall die Meldung mit dem erwarteten Ergebnis und Abort-Code — 1018h fehlt (SuppliedDefault), 1800h:02 = F5h (Corrected, 0609 0030h, FEh bleibt), 1801h:01 mit Bit 29 (PdoDisabled), 1A02h:01 auf ein nicht existierendes Objekt (PdoDisabled, 0602 0000h), 1804h/1A04h (Omitted), 1012h (NotImplemented), INTEGER24 (Omitted), doppelte Node-ID in 1016h (Corrected, 0604 0043h), abweichendes 1200h:01 (NotImplemented), 1010h:02 (Omitted) — und das OD trägt jeweils den korrigierten Wert. | Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 2); Abort-Codes nach CiA 301 §7.2.4.3.17; #132 |
| FR-CO-027 | Das System MUSS die Zugriffsrechte, die eine Gerätebeschreibung für die PDO-Records 1400h–1403h, 1600h–1603h, 1800h–1803h und 1A00h–1A03h angibt, auf dem Bus durchsetzen — ein als `rw` beschriebenes Record ist per SDO schreibbar, auch ohne die Option `WritableCommunicationParameters`, ein als `ro` beschriebenes Objekt bleibt `ro` (0601 0002h) — und das `PDOMapping`-Attribut der Datei als Mappbarkeit des Objekts übernehmen, so dass ein Objekt mit `PDOMapping=0` beim Mapping nach FR-CO-020 mit 0604 0041h abgewiesen wird. | Must | Virtual-Loopback-Integrationstest (`CanOpenDeviceDescriptionTests`): mit `device.eds` und ohne die Option schreibt ein Master 1800h:01 und 1A00h:00/01 per Re-Mapping-Prozedur und liest 1A00h:01 zurück; 2002h (`ro`) → 0601 0002h; `PDOMapping=0` von 2002h steht als `PdoMappable = false` im OD, `PDOMapping=1` von 2000h als `true`. | Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 13, 26); CiA 301 §7.5.2.36; #132 |
| FR-CO-028 | Mit einer Gerätebeschreibung MUSS der Knoten deren Werte als Einschaltwerte führen: „load“ (1011h:01, `RestoreDefaultParameters`) und ein NMT Reset Communication stellen 1000h–1FFFh auf die beschriebenen — oder die seit dem letzten „save“ gespeicherten — Werte zurück, ein NMT Reset Node zusätzlich die Applikationsobjekte der Datei; Reset Communication lässt die Applikationsobjekte unverändert. | Must | Virtual-Loopback-Integrationstest (`CanOpenDeviceDescriptionTests`): nach lokalem Schreiben von 2000h und 1017h liefert Reset Communication 1017h = 200 (Datei) und 2000h unverändert, Reset Node 2000h = 1234h (Datei); „save“ nach 1017h = 300, dann „load“ und Reset Communication → 1017h = 200. | CiA 301 §7.3.2.2.1 (Einschaltwerte), §7.5.2.14 („load“), §7.2.8.2.1.5/§7.2.8.2.1.6 (Reset Node / Reset Communication); Architekturentscheidung (Scope-Dokument 2026-09-15, Posten 1); #132 |

#### 4.3.3 J1939 (Applikation)

Ist-Zustand (15.09.2026): ausgeliefert als `CanKit.Pro.J1939`, aufbauend auf `CanKit.Pro.J1939Tp`.
...
| FR-J1939-002 | Das System MUSS dem Applikationsentwickler die Möglichkeit bieten, einzelne SPN-Werte — vorzeichenlos oder im Zweierkomplement — aus einem empfangenen PGN-Payload gemäß konfigurierbarer Skalierung/Offset zu extrahieren. Die Indikatorbereiche nach SAE J1939-71 §5.1.1 (*not available*, *error*, reserviert, parameterspezifisch) MÜSSEN dabei als solche gemeldet werden und DÜRFEN NICHT als physikalischer Messwert ausgegeben werden. | Must | Unit-Test: SPN-Extraktion mit bekannten Skalierungsfaktoren liefert erwartete physikalische Werte; Known-Answer-Tests je Indikatorbereich und an dessen Grenzen für 1-, 2- und 4-Byte-Felder sowie für Sub-Byte- und vorzeichenbehaftete SPN. | SAE J1939-71 §5.1.1, §5.1.3 |
...
**Grundprinzip:** Jede `Must`-Anforderung MUSS mindestens durch Unit- oder Virtual-Loopback-Integrationstest abgedeckt sein, bevor die jeweilige Ebene als „fertig“ gilt. HIL-Tests ergänzen für produktionskritische L4-Szenarien, sind aber wegen Hardwareabhängigkeit nicht Teil des Standard-CI-Gates. Diese Regel wird seit dem 15.09.2026 maschinell geprüft: `eng/verify-requirements-traceability.py` meldet jede `Must`-Anforderung, die kein Test erwähnt, und läuft als CI-Job `requirements traceability` auf jeder Pull Request. Wie viele das sind, sagt der Skriptlauf — hier steht die Zahl bewusst nicht, damit sie nicht veralten kann.

---

## 8. Traceability-Matrix

**Pfadangaben mit `src/core/…` oder `src/adapters/…` zeigen auf L0/L1 im Upstream-Repository [`pkuyo/CanKit`](https://github.com/pkuyo/CanKit)** (konsumiert als NuGet-Paket, Tag in `eng/Dependencies.props`) — in *diesem* Repository existieren sie nicht. Alles unter `src/CanKit.Pro.*` ist lokal.

Verweise auf Architektur-Bausteine nutzen die in Abschnitt 2.1 definierten Schichtnamen L0–L4 und die Bausteinnamen aus `docs/architecture/arc42-CanKit.Pro.md`, das inzwischen vorliegt. Die früheren *geplant*-Markierungen sind aufgelöst: jede Zeile nennt das Paket, das die Anforderung heute trägt.

| Anforderung(en) | Architektur-Baustein (L0–L4) | Verifikation |
|---|---|---|
| FR-RAW-001..005 | L2 – *Frame-Ownership-Vertrag*, aufbauend auf L1 `CanFrame`/`CanFrameView` (`src/core/CanKit.Abstractions/API/Can/Definitions/CanFrame.cs`) | Unit-Test, Virtual-Loopback-Integrationstest |
...
| FR-CO-001..028 | L4 – CANopen-Stack, ausgeliefert als `CanKit.Pro.CANopen` (`CanOpenNode` auf L2-Demultiplexing; Kommunikationsprofil und PDO-Engine werden aus dem Objektverzeichnis abgeleitet, `CanOpenNode.CommunicationProfile.cs` / `CanOpenNode.Pdo.cs`; das Objektverzeichnis aus einer EDS/DCF, `CanOpenNode.DeviceDescription.cs` auf `EdsDcfNet`) | Virtual-Loopback-Integrationstest, Unit-Test mit virtueller Uhr, HIL-Stichprobe |
| FR-J1939-001..007 | L4 – J1939-Applikationsschicht, ausgeliefert als `CanKit.Pro.J1939` (`J1939NodeImpl`) | Virtual-Loopback-Integrationstest, HIL-Stichprobe |
...
4. **Teilweise erledigt (21.09.2026): der Zuschnitt gegen die Normen.** Für CANopen ist die Geräterolle entschieden (`docs/reviews/2026-09-15-canopen-scope.md`, 16.09.2026) und in 4.3.2 als FR-CO-013..028 übersetzt; die Master-/Tool-Rolle hat am 26.09. ihre Entscheidungen in `docs/reviews/2026-09-26-canopen-master-tool-scope.md` (#131), darunter Flying Master als Pflicht. Die Wahl selbst ist mit #164 unter einer angenommenen CiA-302-2-Bindung umgesetzt; Frage 4 (Ausgabe und Abschnitt) und Frage 5 (Boot-up-Manager) bleiben offen. Frage 3 dort (Live-Mapping und COB-ID) ist beantwortet und in `ObserveForeignPdoAsync` (#163) umgesetzt. Ausformulierte FR-CO-Zeilen zur Master-Rolle fehlen, solange die übrigen Entscheidungsfragen offen sind. Offen bleiben ISO 14229-1 und die übrigen Runden aus `docs/reviews/2026-09-15-norm-gap.md`.
