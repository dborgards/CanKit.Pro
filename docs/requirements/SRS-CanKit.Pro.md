...
Ist-Zustand (15.09.2026): ausgeliefert als `CanKit.Pro.CANopen`.
...
**Grundprinzip:** Jede `Must`-Anforderung MUSS mindestens durch Unit- oder Virtual-Loopback-Integrationstest abgedeckt sein, bevor die jeweilige Ebene als „fertig“ gilt. HIL-Tests ergänzen für produktionskritische L4-Szenarien, sind aber wegen Hardwareabhängigkeit nicht Teil des Standard-CI-Gates. Diese Regel wird seit dem 15.09.2026 maschinell geprüft: `eng/verify-requirements-traceability.py` meldet jede `Must`-Anforderung, die kein Test erwähnt. Die Verdrahtung als CI-Job folgt in einer eigenen Pull Request; bis dahin ist das Skript von Hand zu starten. Wie viele das sind, sagt der Skriptlauf — hier steht die Zahl bewusst nicht, damit sie nicht veralten kann.
...
| FR-CO-001..012 | L4 – CANopen-Stack, ausgeliefert als `CanKit.Pro.CANopen` (`CanOpenNode` auf L2-Demultiplexing) | Virtual-Loopback-Integrationstest, HIL-Stichprobe |
...
4. **Offen: der Zuschnitt gegen die Normen.** Dieses Dokument spezifiziert 12 Anforderungen für die gesamte CiA 301 und 12 für ISO 14229-1. Ob das der beabsichtigte Produktumfang ist, ist eine Entscheidung, die noch aussteht — die Grundlage dafür liegt in `docs/reviews/2026-09-15-norm-gap.md`.
