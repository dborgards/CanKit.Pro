...
- `ResponsePending_Restarts_P2Star_And_Returns_Final_Response` — `delayBetween` 60 ms against `P2StarClientMax` of 1 s, and the 60 ms is what pushes the transfer past `P2ClientMax` of 120 ms. Zeroing it would drop the property. Margin to P2* is about 940 ms per gap. Not the race #117 removed. The first 0x78 against that 120 ms, and against the 100 ms on `Client_Times_Out_With_P2Star_When_Ecu_Sends_Only_ResponsePending`, was measured for #118: 8 CPU burners on 4 cores, both budgets lowered together. No P2 timeout at 25 ms or above (stock values included, and the unmodified class stayed green). The timeout appears at 20 ms and is certain at 5 ms. Hypothesis rejected; the budgets stay.
