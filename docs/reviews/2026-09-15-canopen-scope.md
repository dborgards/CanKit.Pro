...
| 23 | **Die Regel hinter 20, 21 und 22:** für jedes Kommunikationsobjekt, das der Knoten anbietet, stimmen OD und Laufzeit in **beiden Richtungen** und in **beiden Pfaden** überein | **Architektur** | Ein **angenommener** SDO-Schreibzugriff erreicht den Dienst, und jede API, die den Dienst ändert, schreibt den Wert ins OD zurück. **„Angenommen" ist die halbe Regel:** auf einem `ro`-Eintrag endet der Download an der Zugriffsprüfung mit `0601 0002h` (Posten 17), und das ist das richtige Verhalten, keine Lücke — sonst machte diese Zeile die Records aus den Posten 3, 4 und 16 beschreibbar, die sie ausdrücklich `ro` setzen. Bisher bekannte Instanzen: `1006h`/SYNC-Produzent (20), `1016h`/`1017h`/Heartbeat (21), `1001h`/EMCY (22), PDO-Records (13/14/16). Die Liste ist hier dreimal in Folge je Dienst nachgezogen worden — sie steht als Regel, damit der nächste Dienst nicht als vierter Befund auffällt |
