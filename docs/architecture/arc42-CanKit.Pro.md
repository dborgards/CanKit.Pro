...
> `FR-RAW-*`, `FR-TP-*`, `FR-UDS-*`, `FR-CO-*`, `FR-J1939-*`, `FR-VENDOR-*`, `NFR-*`, `CON-*`. Die
...
| **L4** | Anwendungsprotokoll-Ebene | **NEU / Ziel** | UDS, CANopen, J1939-App, VENDOR-Privatprotokoll |
...
Anwendungsprotokolle (ISO-TP, UDS, J1939, CANopen, VENDOR) aufsetzen können.
...
| **Protokoll-Autor** (ISO-TP/UDS/J1939/CANopen/VENDOR) | Verbindlicher Ownership-Vertrag, Multi-Consumer-Demux, TX-Confirm, definiertes Threading-Modell (→ L2). |
...
flowchart TB
    subgraph App["Anwendungsprozess (.NET)"]
        ...
        subgraph CanKitLib["CanKit (Bibliothek, in-process)"]
            L4["L4 Anwendungsprotokolle<br/>UDS / CANopen / J1939 / VENDOR  (NEU)"]
...
| L4 Anwendungsprotokolle | NEU | Diagnose-/Applikationssemantik auf L3/L2. | (neu) protokollspezifisch | `FR-UDS-*`, `FR-CO-*`, `FR-J1939-*`, `FR-VENDOR-*` |
...
| **VENDOR** | Hier: herstellerspezifisches Privatprotokoll (Ziel-L4). |
