...
* P2 and P2* end with the **first frame** of the response (ISO 14229-2), not its last: a
  multi-frame response whose First Frame arrived inside the budget is waited for beyond it,
  and the remainder of the transfer is bounded by the ISO-TP `NCr` timer instead. A 4 KB
  record paced at STmin 5 ms takes seconds on the wire and is not a P2 timeout.
