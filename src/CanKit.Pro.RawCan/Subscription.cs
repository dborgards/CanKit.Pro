...
namespace CanKit.Pro.RawCan
{
    ...
    internal sealed class Subscription : ISubscription
    {
            ...
            int capacity)
        {
            ...
            // Bounded + DropOldest gives the same non-blocking fan-out semantics CanKit's own L1
            // RX pipe (AsyncFramePipe) uses. We keep a local Channel rather than reusing that pipe
            // so Dispose can Complete the writer and end the async enumerator deterministically
            // (FR-RAW-012); AsyncFramePipe does not expose graceful completion.
